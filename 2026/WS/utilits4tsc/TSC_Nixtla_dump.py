...
def plot_series_v2(
    ...
    if anomalies_df is not None:
        ...
        for i, uid in enumerate(plotting_order):
            ...
            if not points.empty:
                # === Логика вычисления ширины полоски ===
                if anomaly_width is not None:
                    # Определяем, являются ли данные датами
                    ...
                    if isinstance(anomaly_width, str):
                        ...
                        if is_datetime:
                            # Если данные - даты, а ширина - число, считаем число как ДНИ
                            half_w = pd.Timedelta(days=anomaly_width) / 2
                        else:
                            # Если данные - числа, используем как есть
                            half_w = anomaly_width / 2
...
def plot_series_v3(
    ...
    marker_size: int = 50,
    anomaly_width: Optional[Union[str, float, pd.Timedelta]] = None,
    anomaly_models: Optional[List[str]] = None,
):
    # Если не matplotlib - вызываем оригинал
    if engine != "matplotlib":
        return uf_plot_series(
            df=df, forecasts_df=forecasts_df, models=models, level=level,
            max_insample_length=max_insample_length, plot_anomalies=plot_anomalies,
    ...
    # === Логика отбора ID (без изменений) ===
    all_ids = set()
    if df is not None: all_ids.update(df[id_col].unique())
    if forecasts_df is not None: all_ids.update(forecasts_df[id_col].unique())
    if anomalies_df is not None: all_ids.update(anomalies_df[id_col].unique())
    ...
    if ax is not None:
        # Если внешний ax, то сложнее управлять удалением колонок, вызываем как есть
        return uf_plot_series(
            df=df_filtered, forecasts_df=forecasts_filtered, models=models, level=level,
            max_insample_length=max_insample_length, plot_anomalies=plot_anomalies,
            engine=engine, palette=palette, id_col=id_col, time_col=time_col,
    ...
    # === ГЛАВНОЕ ИСПРАВЛЕНИЕ: Удаляем колонки аномалий из данных для ЛИНИЙ ===
    # uf_plot_series не должна видеть колонки iforest/lof, чтобы не рисовать их как линии y=0
    cols_to_drop = anomaly_models if anomaly_models else []
    
    def safe_drop(data, cols):
        if data is None: return None
        return data.drop(columns=[c for c in cols if c in data.columns], errors='ignore')

    df_for_lines = safe_drop(df_filtered, cols_to_drop)
    fcst_for_lines = safe_drop(forecasts_filtered, cols_to_drop)
    
    # === 1. Рисуем линии (прогнозы/факт) ===
    uf_plot_series(
        df=df_for_lines,
        forecasts_df=fcst_for_lines,
        models=models,
        level=level,
        max_insample_length=max_insample_length,
        plot_anomalies=plot_anomalies,
    ...
    # === 2. Настройки для аномалий ===
    legend_handles = []
    legend_labels = []
    
    # Определяем источник данных для аномалий
    # Приоритет: anomalies_df -> df -> forecasts_df
    source_df = None
    if anomalies_df is not None:
        source_df = anomalies_df[anomalies_df[id_col].isin(selected_ids)].copy()
    elif df_filtered is not None and anomaly_models:
        source_df = df_filtered
    elif forecasts_filtered is not None and anomaly_models:
        source_df = forecasts_filtered

    # === 3. Рисуем аномалии (ТОЧКИ) ===
    if anomaly_models and source_df is not None:
        cmap = plt.get_cmap('Set1')
        anomaly_colors = {model: cmap(i) for i, model in enumerate(anomaly_models)}

        for i, uid in enumerate(plotting_order):
            subset = source_df[source_df[id_col] == uid]
            
            for model_name in anomaly_models:
                if model_name not in subset.columns: continue
                
                # Берем только точки аномалий (-1)
                points = subset[subset[model_name] == -1]
                
                if not points.empty:
                    color = anomaly_colors[model_name]
                    
                    # Рисуем полоски (если задана ширина)
                    if anomaly_width is not None:
                        is_datetime = pd.api.types.is_datetime64_any_dtype(points[time_col])
                        half_w = None
                        if isinstance(anomaly_width, str): half_w = pd.to_timedelta(anomaly_width) / 2
                        elif isinstance(anomaly_width, pd.Timedelta): half_w = anomaly_width / 2
                        elif isinstance(anomaly_width, (int, float)):
                            half_w = pd.Timedelta(days=anomaly_width) / 2 if is_datetime else anomaly_width / 2
                        
                        if half_w is not None:
                            for t in points[time_col]:
                                axes[i].axvspan(t - half_w, t + half_w, color=color, alpha=0.2, zorder=0)
                    ...
                    sc = axes[i].scatter(
                        ...
                        label=model_name, zorder=100
                    )
                    
                    # Сохраняем для легенды (достаточно одного раза на фигуру)
                    if model_name not in legend_labels:
                        legend_handles.append(sc)
                        legend_labels.append(model_name)

    # === 4. Финальная легенда ===
    # Обновляем легенды на каждом графике, добавляя наши точки
    for i in range(n_plots):
        handles, labels = axes[i].get_legend_handles_labels()
        
        # Словарь уберет дубликаты, если что-то совпадет
        by_label = dict(zip(labels, handles))
        
        # Добавляем наши хэндлы аномалий
